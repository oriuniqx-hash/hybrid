...
import { Bookmark, Check, Eye, Heart, MessageCircle, Pin, Send, Tag } from 'lucide-react'
...
export default function PostCard({post,onGuestAction}:{post:Post;onGuestAction?:()=>void}){
 const supabase=useMemo(()=>createSupabaseBrowserClient(),[])
 const [userId,setUserId]=useState<string|null>(null),[liked,setLiked]=useState(false),[saved,setSaved]=useState(false),[likes,setLikes]=useState(post.likes_count||0),[comments,setComments]=useState<Comment[]>([]),[commentText,setCommentText]=useState(''),[showComments,setShowComments]=useState(false),[busy,setBusy]=useState(false),[shared,setShared]=useState(false)
 ...
 async function refreshSocial(){
  ...
  setUserId(id);setLikes(likeCount??0);setComments((commentRows as Comment[]|null)??[])
  if(id){const {data:ownLike}=await supabase.from('likes').select('id').eq('post_id',post.id).eq('user_id',id).maybeSingle();setLiked(Boolean(ownLike))}
  try{setSaved(localStorage.getItem('hybrid:saved:'+post.id)==='1')}catch{}
 }

 useEffect(()=>{
  void refreshSocial()
  const channel=supabase.channel('post-social-'+post.id)
 ...
 async function toggleLike(){
  ...
  if(busy)return
  setBusy(true)
  if(liked)await supabase.from('likes').delete().eq('post_id',post.id).eq('user_id',userId)
  else await supabase.from('likes').insert({post_id:post.id,user_id:userId})
  await refreshSocial();setBusy(false)
 }

 function toggleSave(){
  if(!userId){onGuestAction?.();return}
  const next=!saved;setSaved(next)
  try{if(next)localStorage.setItem('hybrid:saved:'+post.id,'1');else localStorage.removeItem('hybrid:saved:'+post.id)}catch{}
 }

 function toggleComments(){setShowComments(v=>!v)}
 ...
 async function share(){
  const url=typeof window!=='undefined'?window.location.origin+'/dashboard/profile/'+post.user_id:''
  try{
   if(navigator.share)await navigator.share({title:post.title||'HYBRID creative',url})
   else if(navigator.clipboard){await navigator.clipboard.writeText(url);setShared(true);setTimeout(()=>setShared(false),1600)}
  }catch{}
 }
 ...
 return <article className="group mb-5 break-inside-avoid overflow-hidden rounded-[26px] border border-white/10 bg-white/[.035] shadow-[0_18px_80px_rgba(0,0,0,.25)]">
  <div className="relative overflow-hidden bg-black/30">
   {post.type==='reel'?<video controls playsInline preload="metadata" poster={post.thumbnail_url||undefined} src={post.media_url} className="block max-h-[680px] w-full object-cover"/>:<img loading="lazy" src={post.thumbnail_url||post.media_url} alt={post.title||'HYBRID post'} className="block w-full object-cover transition duration-500 group-hover:scale-[1.015]"/>}
  ...
  <div className="p-4">
   <div className="flex items-start gap-3"><div className="h-9 w-9 shrink-0 overflow-hidden rounded-xl bg-white/10">{post.profiles?.avatar_url&&<img loading="lazy" src={post.profiles.avatar_url} alt="" className="h-full w-full object-cover"/>}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-bold">@{post.profiles?.username||'creator'}</p><span className="text-[9px] uppercase tracking-[.16em] text-white/25">Campaign asset</span></div><p className="mt-1 truncate text-xs text-white/40">{post.title||'Untitled creative'}</p></div></div>
   {post.description&&<p className="mt-3 text-sm leading-6 text-white/55">{post.description}</p>}
   <div className="mt-4 flex items-center gap-2">
    <button onClick={toggleLike} disabled={busy} aria-label={liked?'Unlike':'Like'} className={'rounded-xl px-3 py-2 text-xs font-bold transition '+(liked?'bg-[#E60023]/15 text-[#ff5870]':'bg-white/5 text-white/55 hover:bg-white/10 hover:text-white')}><Heart size={14} className="mr-1 inline"/> {likes}</button>
    <button onClick={toggleComments} aria-label="Comments" className="rounded-xl bg-white/5 px-3 py-2 text-xs font-bold text-white/55 hover:bg-white/10 hover:text-white"><MessageCircle size={14} className="mr-1 inline"/> {comments.length}</button>
    <button onClick={toggleSave} aria-label={saved?'Remove save':'Save'} className={'rounded-xl p-2 transition '+(saved?'bg-[#833AB4]/20 text-white':'bg-white/5 text-white/45 hover:text-white')}>{saved?<Check size={14}/>:<Bookmark size={14}/>}</button>
    <button onClick={share} aria-label="Share" className="ml-auto rounded-xl bg-white/5 p-2 text-white/45 hover:text-white">{shared?<Check size={14}/>:<Send size={14}/>}</button>
   </div>
   {showComments&&<div className="mt-4 border-t border-white/10 pt-4"><div className="max-h-52 space-y-3 overflow-auto">{comments.length===0?<p className="text-xs text-white/35">No comments yet.</p>:comments.map(c=><div key={c.id} className="text-xs"><b>@{c.profiles?.username||'user'}</b> <span className="text-white/55">{c.content}</span></div>)}</div>{userId?<div className="mt-3 flex gap-2"><input value={commentText} onChange={e=>setCommentText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void addComment()}} placeholder="Add a campaign note..." className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs outline-none"/><button onClick={addComment} disabled={busy||!commentText.trim()} className="rounded-xl bg-gradient-to-r from-[#E60023] via-[#C13584] to-[#833AB4] px-3 py-2 text-xs font-bold">Post</button></div>:<button onClick={()=>onGuestAction?.()} className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 py-2 text-xs font-semibold text-white/60 hover:text-white">Sign in to comment</button>}</div>}
  </div>
 </article>
}
