    ...
    <main className="min-h-screen bg-[#05070c] text-white">
      <header className="sticky top-0 z-40 border-b border-white/5 bg-[#05070c]/90 px-3 py-3 backdrop-blur-2xl lg:px-6">
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button data-notification-trigger onClick={() => setNotifications(v => !v)} className="rounded-xl border border-white/10 bg-white/5 p-2.5 text-white/55 hover:text-white"><Bell size={18}/></button>
      ...
      <div className="lg:pl-20">
        {view === 'landing' && !user ? <Landing onExplore={() => setView('explore')} onAuth={() => setAuthOpen(true)}/> :
         view === 'profile' ? <Profile profile={profile} posts={visiblePosts.filter(p => p.user_id === user?.id)} onCreate={() => restricted(() => { window.location.href='/dashboard/create' })} onSignOut={signOut} onGuestAction={() => setGuestPrompt(true)}/> :
      ...
      {notifications && <div data-notifications className="fixed right-4 top-20 z-50 w-80 rounded-3xl border border-white/10 bg-[#0d111a]/95 p-5 shadow-2xl backdrop-blur-2xl"><div className="flex items-center justify-between"><b className="text-sm">Notifications</b><button onClick={() => setNotifications(false)}><X size={16}/></button></div><div className="mt-5 rounded-2xl border border-white/5 bg-white/5 p-4 text-xs text-white/55">You're all caught up. New likes, comments, and messages will appear here.</div></div>}
